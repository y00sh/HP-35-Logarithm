"""
@title Logarithm base 2 function
@license GNU Affero General Public License v3.0
@author Yoosh
@notice Inspired by HP-35 calculator's algorithm
        - `log2` (`external` `pure` function)
"""
...
def log2(x: decimal) -> decimal:
    """
    @dev Is accurate to 10 decimal places but needs to shift 
         the fixed-point 10 places before performing arithmetic
    @notice Note that the result is rounded down towards zero. 
    @param x positive decimal type from 
           0.0000000001 to 18707220957835557353007165858768422651595.9365500927
    @return decimal type
    """
    m: decimal = x
    ...
    for j in range(34): 
        ...
        #print(m)
        if m >= A[j]:
            #m = (m / (A[j]/1000.0) ) * 10000000.0
            m = (m * 10000000000.0)/A[j]
            #m = (m / A[j]) * 10000000000.0
            q[j] = 1
